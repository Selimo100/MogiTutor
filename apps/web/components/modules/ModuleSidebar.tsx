...
interface ModuleSidebarProps {
  ...
  export function ModuleSidebar({ moduleCode, tutorId, data, competencies }: ModuleSidebarProps) {
    const pathname = usePathname();
    const compMap = new Map((competencies || []).map((c: any) => [c.code, c]));
                      ...
                      {band.rows.map((row, idx) => (
                          row.competencies.map((comp) => {
                              ...
                              const status = dbComp?.status || 'open';
                                ...
                                let statusClasses = "text-muted-foreground hover:text-foreground hover:bg-muted/50";
                                if (status === 'done') {
                                    statusClasses = "bg-green-500/10 text-green-700 dark:text-green-400 hover:bg-green-500/20";
                                } else if (status === 'in_progress') {
                                    statusClasses = "bg-blue-500/10 text-blue-700 dark:text-blue-400 hover:bg-blue-500/20";
                                }
  
                                if (isActive) {
                                    // Combine active state with status colors - if colored, add bold and maybe a border or deeper bg
                                    if (status !== 'open') {
                                        statusClasses = cn(statusClasses, "font-bold ring-1 ring-inset ring-primary/50");
                                    } else {
                                        statusClasses = "text-primary font-medium bg-primary/5";
                                    }
                                }
